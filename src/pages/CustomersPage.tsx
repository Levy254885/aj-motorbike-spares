...
export default function CustomersPage() {
  ...
  const load = () => listCustomers().then(setCustomers).catch(console.error).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? <div className="h-24 animate-pulse rounded-lg bg-zinc-100" /> : customers.length === 0 ? (
        <p className="rounded-lg border border-dashed p-6 text-center text-sm text-zinc-500">No customers yet. Walk-in is allowed on POS.</p>
      ) : (
